...
bot = commands.AutoShardedBot(command_prefix=config.command_prefix, description="A multi-purpose Puggle Bot made by yours truely, pug#6748", pm_help=None) 
    ...
    if name is not None:
        if config.default_status_type == "stream":
            if config.default_status_name is None:
                ...
                os._exit(1)
            status = discord.Activity(name=name, url="http://twitch.tv/1dx_Juuker", type=discord.ActivityType.streaming)
        else:
...
async def on_ready():
    ...
    if config.enable_default_status:
        ...
        await bot.change_presence(activity=discord.Activity(name="logan paul vs ksi", type=discord.ActivityType.streaming), status=discord.Status.idle)
...
async def notifydev(ctx, *, message:str):
    ...
    if isinstance(ctx.channel, discord.DMChannel):
        guild = "`Not send via server command- sent via Private Message!`"
    else:
        guild = "`{}` / `{}`".format(ctx.guild.id, ctx.guild.name)
...
async def stream(ctx, *, name:str):
    ...
    if lock_status:
        if not ctx.author.id == config.owner_id and not ctx.author.id in config.dev_ids:
            ...
            return
    await bot.change_presence(activity=discord.Activity(name=name, type=discord.ActivityType.streaming, url="https://www.twitch.tv/1dx_juuker"))
...
@bot.command()
async def codes(ctx):
    await ctx.send("marco makes good codes and you are gay")

@bot.command()
async def gay(ctx):
    """tells someone something nice"""
    await ctx.send("<@474635321993330710> delete ur account")

@bot.command()
async def LightUk(ctx):
    """UK ftw"""
    await ctx.send("You're on your last warning. I'd be careful if I were you. *By the way, the United Kingdom is much superior to any other country, and God save our Queen*, PEASANTS.")
